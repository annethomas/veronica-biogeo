## modifies dtt to plot absolute time
## remaining internal functions copied directly from https://rdrr.io/cran/geiger/src/R/disparity.R#sym-.dtt
## and https://rdrr.io/cran/geiger/src/R/utilities-misc.R
dtt.abstime=function (phy, data, index = c("avg.sq", "avg.manhattan", 
                               "num.states"), mdi.range = c(0, 1), nsim = 0, CI = 0.95, 
          plot = TRUE, calculateMDIp = F) 
{
  disp = match.arg(index, c("avg.sq", "avg.manhattan", 
                            "num.states"))
  ...
  ltt <- c(max(ltt),ltt)
  #print(ltt)
  ltt.rel <- c(0, (max(ltt) - ltt)/max(ltt))
  #print(ltt.rel)
  if (disp == "num.states") {
  ...
  if (plot) {
    plot(ltt, dtt.data, xlab = "Time (Ma)", ylab = "Avg. subclade disparity", 
