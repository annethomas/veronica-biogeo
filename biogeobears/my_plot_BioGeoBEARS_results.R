...
## added tipboxes_text t/f option, label.font face,

my_plot_BioGeoBEARS_results=function(results_object, analysis_titletxt=NULL, 
                                  addl_params=list(), plotwhat="text", label.offset=NULL, label.font=1,
                                  ...
                                  skiptree=FALSE, show.tip.label=TRUE, tipcol="black",edge.width=1,
                                  ...
                                  tipboxes_TF=TRUE, tipboxes_text=TRUE, tiplabel_adj=c(0.5), no.margin=FALSE, 
                                  xlims=NULL, ylims=NULL)
{
  ...
  if (skiptree != TRUE)
  {
    ...
    #######################################################
    plot(tr_pruningwise, x.lim=xlims, y.lim=ylims, show.tip.label=FALSE, label.offset=label.offset, cex=tipcex, no.margin=no.margin, root.edge=root.edge,edge.width=edge.width)
    ...
    if (show.tip.label == TRUE)
    {
      ...
      if (skiplabels == FALSE)
      {
        tiplabels(text=tiplabels_to_plot, tip=tips, cex=tipcex, adj=0, bg="white", frame="n", pos=4, offset=label.offset, col=tipcol,font=label.font)	# pos=4 means labels go to the right of coords
    ...
    if (skiplabels == FALSE)
    {
      ...
      if (tipboxes_TF == TRUE)
      {
        if(tipboxes_text){
          ...
          tiplabels(text=NULL,pch=21, tip=tips, bg=cols_byNode[tips], cex=pie_tip_statecex, adj=tiplabel_adj)
        }
...
## functions copied from bgb to make accessible to this version
...
rcpp_areas_list_to_states_list=function (areas = c("A", "B", "C"), maxareas = length(areas), 
          include_null_range = TRUE) 
{
  ...
  nstates = numstates_from_numareas(numareas = length(areas), 
                                    maxareas = maxareas, include_null_range = include_null_range)
  R_areas_indices = seq(0, length(areas) - 1, 1)
  R_states_list = as.list(rep(-1, times = nstates))
  R_maxareas = maxareas
  R_include_null_range = include_null_range
  R_states_list = .Call("cpp_areas_list_to_states_list", 
                        as.integer(R_areas_indices), as.integer(R_maxareas), 
                        as.logical(R_include_null_range), PACKAGE = "cladoRcpp")
  if (include_null_range == TRUE) {
