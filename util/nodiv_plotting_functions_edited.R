plot_nodes_tips_phylo <- function(node.variable, tip.variable,tree, label, main = deparse(substitute(variable)), zlim = NULL, col , show.legend = TRUE, sig.cutoff, nodes, tips, roundoff= TRUE, show.tip.label = NULL, cex = NULL, ...)
{
  if (!length(node.variable) == Nnode(tree))
    stop("The length of the node.variable vector must be the same length as the number of nodes on the tree")
  
  if (is.null(show.tip.label)) show.tip.label <- isTRUE(Ntip(tree) < 50)
  
  plotvar=c(node.variable,tip.variable)
  if(missing(label)){
    label=plotvar
  }
  ...
  node_index[is.na(node.variable)] <- FALSE
  
  
  if (missing(tips)) tip_index = rep(TRUE, Ntip(tree)) else {
    tip_index <- rep(FALSE, Ntip(tree))
    tip_index[tips] <- TRUE
  }
  
  tip_index[is.na(tip.variable)] <- FALSE
  
  if (!missing(sig.cutoff))
    node_index[node.variable <= sig.cutoff] <- FALSE
  
  nodes <- nodenumbers(tree)[node_index]
  tips <- tree$tip.label[tip_index]
  #node.plotvar <- node.variable[node_index]
  #tip.plotvar <- tip.variable[tip_index]
  #label <- label[node_index] ## deal with adding tips later
  ...
  title(main)
  nodelabels(pch = 16, node = nodes, col = create.cols(plotvar[1:length(nodes)], col, zlim), cex = sizes[1:length(nodes)])
  nodelabels(text = as.character(label[1:length(nodes)]), node = nodes, cex = 0.6, frame = "none") 
  tiplabels(pch = 16,  col = create.cols(plotvar[(length(nodes)+1):length(plotvar)], col, zlim), cex = sizes[(length(nodes)+1):length(plotvar)])
  tiplabels(text = as.character(label[(length(nodes)+1):length(plotvar)]), cex = 0.6, frame = "none") 
